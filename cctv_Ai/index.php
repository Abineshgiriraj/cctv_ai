...
<main id="overview">
    ...
    <section class="content">
        <div class="hero-row">
            <div>
                <p class="muted">Live camera monitoring, vehicle crossing counts, helmet violations and time-based reports.</p>
            </div>
        ...
        <div class="operator-summary">
            <article class="operator-kpi">
                <span class="kpi-icon"><i class="bi bi-camera-video"></i></span>
                <div><small>ACTIVE CAMERAS</small><strong id="activeCameraCount">0 / <?= count($cameras) ?></strong><p id="cameraNetworkText">Checking camera streams</p></div>
            </article>
            <article class="operator-kpi">
                <span class="kpi-icon"><i class="bi bi-car-front-fill"></i></span>
                <div><small>VEHICLES COUNTED TODAY</small><strong id="todayVehicleCount">0</strong><p>Unique line-crossing events in MySQL</p></div>
            </article>
            <article class="operator-kpi">
                <span class="kpi-icon"><i class="bi bi-person-x-fill"></i></span>
                <div><small>NO-HELMET TODAY</small><strong id="todayNoHelmetCount">0</strong><p>Confirmed violation records</p></div>
            </article>
            <article class="operator-kpi">
                <span class="kpi-icon"><i class="bi bi-cpu"></i></span>
                <div><small>AI STATUS</small><strong id="aiDetectionStatus">STARTING</strong><p id="aiDetectionText">YOLO + ByteTrack</p></div>
            </article>
        </div>
        ...
        <section class="section-block" id="live-monitoring">
            <div class="section-heading">
                <div><h2>Live Monitoring</h2><p>Click any camera to open a large view. AI Tracking is the default stream.</p></div>
            </div>
                ...
                <?php foreach ($cameras as $camera): ?>
                    <article class="operator-camera-card" data-camera-ip="<?= e($camera['ip']) ?>" data-camera-number="<?= (int)$camera['number'] ?>">
                        <div class="operator-camera-head">
                            <div class="operator-camera-title">
                                <span class="cam-dot"></span>
                                <div><h3><?= e($camera['name']) ?></h3><p><?= e($camera['ip']) ?></p></div>
                            </div>
                        ...
                        <div class="operator-camera-frame camera-frame">
                            <img
                                ...
                                data-stream-mode="ai"
                                alt="CCTV stream <?= e($camera['name']) ?>"
                            >
                            <div class="stream-message" id="streamMessage<?= (int)$camera['number'] ?>">
                                <i class="bi bi-camera-video"></i><b>WAITING FOR STREAM</b><span>Starting AI feed...</span>
                            </div>
                        ...
                        <div class="operator-camera-toolbar">
                            <div class="operator-mode-switch">
                                <button type="button" class="active" id="aiMode<?= (int)$camera['number'] ?>" onclick="switchCameraMode(<?= (int)$camera['number'] ?>,'ai')"><i class="bi bi-bounding-box"></i> AI</button>
                                <button type="button" id="rawMode<?= (int)$camera['number'] ?>" onclick="switchCameraMode(<?= (int)$camera['number'] ?>,'raw')"><i class="bi bi-camera-video"></i> Raw</button>
                            </div>
        ...
        <section class="section-block" id="reports">
            <div class="section-heading">
                <div><h2>Vehicle Count Reports</h2><p>Filter by day, date range, exact time range and camera. Values come from stored crossing events.</p></div>
            </div>
            <article class="report-panel">
                <div class="report-filter-grid">
                    ...
                    <div class="report-field">
                        <label for="reportCamera">CAMERA</label>
                        <select id="reportCamera">
                    ...
                    <div class="report-actions">
                        <button type="button" class="secondary" id="btnTodayReport">Today</button>
                        <button type="button" class="primary" id="btnRefreshReport"><i class="bi bi-search"></i> Apply</button>
                    </div>
                </div>
                ...
                <div class="report-table-grid">
                    <div class="data-card">
                        <div class="data-card-head"><h3><i class="bi bi-calendar3"></i> Day-wise Count</h3><small>One row per day</small></div>
                        <div class="table-scroll"><table class="ops-table" id="tblDailyReport"><thead><tr><th>Date</th><th>Motorcycles</th><th>Cars</th><th>Buses</th><th>Trucks</th><th>Bicycles</th><th>Total Vehicles</th></tr></thead><tbody><tr><td colspan="7" class="empty-row">Loading...</td></tr></tbody></table></div>
                    </div>
                    <div class="data-card">
                        <div class="data-card-head"><h3><i class="bi bi-clock-history"></i> Time-wise Count</h3><small>Hourly totals inside selected period</small></div>
                        <div class="table-scroll"><table class="ops-table" id="tblHourlyReport"><thead><tr><th>Date</th><th>Time</th><th>Motorcycles</th><th>Cars</th><th>Buses</th><th>Trucks</th><th>Bicycles</th><th>Total Vehicles</th></tr></thead><tbody><tr><td colspan="8" class="empty-row">Loading...</td></tr></tbody></table></div>
                    </div>
                    <div class="data-card">
                        <div class="data-card-head"><h3><i class="bi bi-camera-video"></i> Camera-wise Count</h3><small>Stored crossings by camera</small></div>
                        <div class="table-scroll"><table class="ops-table" id="tblCameraReport"><thead><tr><th>Camera</th><th>IP</th><th>Motorcycles</th><th>Cars</th><th>Buses</th><th>Trucks</th><th>Bicycles</th><th>Total Vehicles</th></tr></thead><tbody><tr><td colspan="8" class="empty-row">Loading...</td></tr></tbody></table></div>
                    </div>
                </div>
            </article>
        </section>
        ...
        <section class="section-block" id="violations">
            <div class="section-heading">
                <div><h2>Recent No-Helmet Violations</h2><p>Confirmed records with evidence; number plate is shown when OCR succeeds.</p></div>
