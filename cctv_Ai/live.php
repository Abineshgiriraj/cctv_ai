...
<section class="section-block" id="live-monitoring">
    <div class="camera-pagination-bar" id="cameraPagination">
        ...
        <div class="camera-pagination-actions">
            ...
            <label for="cameraHealthFilter">Status</label>
            <select id="cameraHealthFilter" aria-label="Filter cameras by health status">
                <option value="">All status</option>
                <option value="online">Online</option>
                <option value="offline">Offline</option>
                <option value="ai_ready">AI processed</option>
                <option value="ai_error">AI error</option>
            </select>

            <div class="camera-picker" id="cameraPicker">
                <button type="button" class="camera-picker-toggle" id="cameraPickerToggle" aria-expanded="false">
                    <i class="bi bi-camera-video"></i>
                    <span id="cameraPickerLabel">Select cameras</span>
                    <i class="bi bi-chevron-down"></i>
                </button>
                <div class="camera-picker-menu" id="cameraPickerMenu">
                    <div class="camera-picker-search">
                        <i class="bi bi-search"></i>
                        <input type="search" id="cameraPickerSearch" placeholder="Search camera, area, IP or channel">
                    </div>
                    <div class="camera-picker-actions">
                        <button type="button" id="cameraSelectVisible">Select filtered</button>
                        <button type="button" id="cameraClearSelection">Clear</button>
                    </div>
                    <div class="camera-picker-list" id="cameraPickerList">
                        <?php foreach ($cameras as $camera): ?>
                            <label
                                class="camera-picker-item"
                                data-search="<?= e(strtolower($camera['name'] . ' ' . $camera['area'] . ' ' . $camera['ip'] . ' ch' . $camera['channel'])) ?>"
                                data-camera-key="<?= e($camera['camera_key']) ?>"
                            >
                                <input type="checkbox" value="<?= e($camera['camera_key']) ?>">
                                <span>
                                    <b><?= e($camera['name']) ?></b>
                                    <small><?= e($camera['area']) ?> · <?= e($camera['ip']) ?> · CH<?= e($camera['channel']) ?></small>
                                </span>
                            </label>
                        <?php endforeach; ?>
                    </div>
                    <div class="camera-picker-footer">
                        <span id="cameraSelectionCount">0 selected · showing normal camera list</span>
                    </div>
                </div>
            </div>
    ...
    <div class="camera-health-summary" id="cameraHealthSummary">
