...
class AccuracyDetector(AdvancedDetector):
    """Precision-first detector with a safe recall fallback for high-angle CCTV."""
    ...
    def __init__(self, config, store, session_id, log):
        ...
        self.helmet_vote_ratio = min(1.0, max(0.5, float(os.getenv("STRICT_NO_HELMET_VOTE_RATIO", "0.75"))))
        self.helmet_conflict_margin = max(0.0, float(os.getenv("HELMET_CONFLICT_MARGIN", "0.12")))
        ...
        self.fallback_vote_ratio = min(1.0, max(0.60, float(os.getenv("FALLBACK_NO_HELMET_VOTE_RATIO", "0.80"))))

        self.road_tile_overlap = min(0.40, max(0.05, float(os.getenv("ROAD_TILE_OVERLAP", "0.18"))))
        ...
        self.log.info(
            "Accuracy mode: rider_no_helmet>=%.2f fallback_no_helmet>=%.2f confirm=%s fallback_confirm=%s road_tiles=%s road_display>=%.2f",
            self.strict_no_helmet_confidence,
    ...
    def _helmet_threshold_for(self, status, source):
        ...
        if source == "bike_fallback":
            return self.fallback_no_helmet_confidence
    ...
    def _helmet_status(self, frame, bike, person):
        ...
        ch, cw = crop.shape[:2]
        max_side = max(ch, cw)
        scale = 6.0 if max_side < 90 else 4.5 if max_side < 140 else 3.0 if max_side < 220 else 2.0 if max_side < 360 else 1.0
        source = cv2.resize(crop, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC) if scale > 1.0 else crop

        infer_conf = min(float(self.cfg.HELMET_CONFIDENCE), self.strict_no_helmet_confidence, self.fallback_no_helmet_confidence)
        try:
            results = model.predict(source, conf=infer_conf, imgsz=int(self.cfg.HELMET_IMGSZ), verbose=False)
        ...
        for result in results or []:
            ...
            for box in boxes:
                try:
                    ...
                    if not status:
                        continue
                    threshold = self._helmet_threshold_for(status, source_name)
        ...
        best_helmet = max(candidates["helmet"], key=lambda row: row["rank"], default=None)
        best_no_helmet = max(candidates["no_helmet"], key=lambda row: row["rank"], default=None)
    ...
    def _helmet_confirmed(self, camera_ip, bike, observation):
        ...
        same = [(conf, vote_source) for vote_status, conf, vote_source in votes if vote_status == status]
        if not same:
            return None

        if status == "no_helmet":
            fallback_track = source == "bike_fallback"
            required = self.fallback_confirm_frames if fallback_track else self.strict_no_helmet_confirm_frames
            required_ratio = self.fallback_vote_ratio if fallback_track else self.helmet_vote_ratio
            threshold = self._helmet_threshold_for(status, source)
            if len(votes) < required or len(same) < required:
                return None
            if len(same) / max(1, len(votes)) < required_ratio:
                return None
            strong_helmet = [
                conf for vote_status, conf, _ in votes
                if vote_status == "helmet" and conf >= float(self.cfg.HELMET_CONFIDENCE) + 0.08
            ]
            if strong_helmet:
                return None
        else:
            threshold = float(self.cfg.HELMET_CONFIDENCE)
    ...
    def _run_road_model(self, camera_ip, frame, model_key, event_type, draw_frame=None):
        ...
        store_threshold = float(self.cfg.ROAD_DAMAGE_CONFIDENCE) if model_key == "road_damage" else float(self.cfg.ADVANCED_CONFIDENCE)
        infer_threshold = self.road_display_confidence if model_key == "road_damage" else store_threshold
        detections = []
        for source, x_offset, y_offset in self._road_tiles(frame):
            try:
                results = model.predict(source, conf=infer_threshold, imgsz=int(self.cfg.ROAD_DAMAGE_IMGSZ), verbose=False)
            except Exception as exc:
            ...
            for result in results or []:
                ...
                for box in boxes:
                    try:
                        ...
                        continue
                    detections.append({"label": label, "confidence": confidence, "box": full_box})

        detections.sort(key=lambda row: row["confidence"], reverse=True)
        kept = []
        for detection in detections:
            if any(
                detection["label"] == existing["label"] and self._iou(detection["box"], existing["box"]) >= 0.45
        ...
        for detection in kept:
            ...
            events.append({"id": event_id, "label": label, "confidence": confidence, "box": full_box})
        return events
